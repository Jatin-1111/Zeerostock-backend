...
ALTER TABLE users 
  ...
  ADD COLUMN IF NOT EXISTS active_role VARCHAR(20) DEFAULT 'buyer',
...
ALTER TABLE users 
  ...
  CHECK (roles <@ ARRAY['buyer', 'supplier', 'admin']);
...
CREATE TABLE IF NOT EXISTS supplier_verification_history (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE INDEX IF NOT EXISTS idx_supplier_verification_status ON supplier_profiles(verification_status);
CREATE INDEX IF NOT EXISTS idx_supplier_verification_history_profile ON supplier_verification_history(supplier_profile_id);
...
FROM users 
WHERE id NOT IN (SELECT user_id FROM buyer_profiles)
