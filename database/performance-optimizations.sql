...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'products' AND column_name = 'text_search_vector') THEN
        ALTER TABLE products ADD COLUMN text_search_vector tsvector;
    END IF;
END $$;

-- 3. Create GIN index for Full Text Search
CREATE INDEX IF NOT EXISTS idx_products_search_vector ON products USING GIN(text_search_vector);
...
CREATE INDEX IF NOT EXISTS idx_products_status_featured ON products(status, is_featured) WHERE is_featured = true;
CREATE INDEX IF NOT EXISTS idx_products_status_trending ON products(status, is_trending) WHERE is_trending = true;
CREATE INDEX IF NOT EXISTS idx_order_items_supplier_order ON order_items(supplier_id, order_id);
CREATE INDEX IF NOT EXISTS idx_orders_created_at ON orders(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_products_price_sort ON products(price_after);
...
CREATE INDEX IF NOT EXISTS idx_shipping_options_product ON shipping_options(product_id);
